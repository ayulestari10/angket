...
class Angket extends CI_Controller{
	...
	function index(){
		$data = array(
			'title'			=> 'ANGKET | Fasilkom Group Discussion II',
			'content'		=> 'Home'
		);
		$this->load->view('includes/template', $data);
	}
	...
	public function adm()
	{
		$data = array(
			'title'			=> 'ANGKET | Fasilkom Group Discussion II',
			'content'		=> 'admin',
			'pernyataan'	=> $this->angket_model->get_all(),
			'kategori'		=> $this->angket_model->get_all_category()
		);
		$this->load->view('includes/template', $data);
	}
	...
	public function detail()
	{
		...
		$data = array(
			'title'			=> 'ANGKET | Fasilkom Group Discussion II',
			'content'		=> 'details',
			'pernyataan'	=> $this->angket_model->get_data_byId($id)
		);
		$this->load->view('includes/template', $data);	
	}
	...
	public function submitAngket()
	{
		 ...
		 */
			...
			for ($i = 1; $i < 50; $i++)
			{
				...
				$vote_value 	= $this->input->post('point' . $i);

				switch ($jurusan)
				{
					case "Teknik Informatika":
						$skor_jurusan[0]++;
						$data_angket[$vote_value][0]++;
						break;

					case "Sistem Informasi":
						$skor_jurusan[1]++;
						$data_angket[$vote_value][1]++;
						break;

					case "Sistem Komputer":
						$skor_jurusan[2]++;
						$data_angket[$vote_value][2]++;
						break;

					case "Diploma":
						$skor_jurusan[3]++;
						$data_angket[$vote_value][3]++;
						break;
				}
				...
				$this->angket_model->update($i, array(
					'jurusan'	=> implode('#', $skor_jurusan),
					'angkatan'	=> implode('#', $skor_angkatan),
					'STS'		=> implode('#', $data_angket['STS']),
					'TS'		=> implode('#', $data_angket['TS']),
					'N'			=> implode('#', $data_angket['N']),
					'S'			=> implode('#', $data_angket['S']),
					'ST'		=> implode('#', $data_angket['ST'])
				));
			...
			$this->kritik_saran_model->insert($ks);

			if ($angkatan == '2012' || $angkatan == '2013')
			{
				$this->load->model('kritik_saran_ta_model');
				$ks_ta = array(
					'dosen_pembimbing'	=> htmlentities($this->input->post('pembimbingTA')),
					'kendala'			=> htmlentities($this->input->post('kendalaTA'))
				);
				$this->kritik_saran_ta_model->insert($ks_ta);
			}

			echo '<div class="alert alert-success">Terima kasih atas partisipasi anda. Semoga dengan aspirasi yang telah diberikan dapat memajukan fakultas ilmu komputer menjadi yang lebih baik lagi.</div>';
	}
